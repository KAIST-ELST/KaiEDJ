...
function nonInt_H_k(Klist,hamiltonian_info,Calculation_mode,DMFT_spin_type)
    ...
    if string(hamiltonian_info.dfttype) == "Wannier90"
        ...
        if ((Calculation_mode == "Jx-DMFT") || (Calculation_mode == "DMFT+MFT") || (Calculation_mode == "Jx0") || (Calculation_mode == "MFT") || (DMFT_spin_type==2) )
            H_k = zeros(size(Klist)[1],size(Klist)[2],size(Klist)[3],sum(hamiltonian_info.scf_r.Total_NumOrbs),sum(hamiltonian_info.scf_r.Total_NumOrbs),2) +
                  zeros(size(Klist)[1],size(Klist)[2],size(Klist)[3],sum(hamiltonian_info.scf_r.Total_NumOrbs),sum(hamiltonian_info.scf_r.Total_NumOrbs),2)*im
        else
            H_k = zeros(size(Klist)[1],size(Klist)[2],size(Klist)[3],sum(hamiltonian_info.scf_r.Total_NumOrbs),sum(hamiltonian_info.scf_r.Total_NumOrbs),1) + 
                  zeros(size(Klist)[1],size(Klist)[2],size(Klist)[3],sum(hamiltonian_info.scf_r.Total_NumOrbs),sum(hamiltonian_info.scf_r.Total_NumOrbs),1)*im
        end
        arg_list=[]
        #load H_k
        ...
        if ((Calculation_mode == "Jx-DMFT") || (Calculation_mode == "DMFT+MFT") || (DMFT_spin_type == 2))
            H_k[:,:,:,:,:,2]= H_k[:,:,:,:,:,1]
        end
...
@everywhere function Cal_Occupation_givenMu_givenK_WOdc(args)
    ...
    dummy = zeros(size(g_H_k[Kind[1],Kind[2],Kind[3],:,:,spin])) +
            zeros(size(g_H_k[Kind[1],Kind[2],Kind[3],:,:,spin]))*im;
    Iden=Matrix{ComplexF64}(I,size(g_H_k[Kind[1],Kind[2],Kind[3],:,:,spin]))

    g_iWnlist_size  = size(g_iWnlist)[1]
    g_iWnlist_max   = g_iWnlist[end]
    MatG_cut_R = GreenF_at_agrid(g_iWnlist_max,Kind,spin,SelfE_at_agrid(g_iWnlist_size,spin),testmu)
    MatG_cut_L = transpose(conj(MatG_cut_R))
    #MatG_cut_L = GreenF_at_agrid(g_iWnlist[1],Kind,spin,SelfE_at_agrid(1,spin),testmu)
    gtilda2 = (g_iWnlist_max*g_iWnlist_max) * (MatG_cut_L+MatG_cut_R)/2.0
    gtilda3 = (g_iWnlist_max*g_iWnlist_max*g_iWnlist_max) * ( (MatG_cut_R-MatG_cut_L)/2.0 - Iden/g_iWnlist_max )
    for w =  1 : (g_iWnlist_size)
        #println("w:",w,"end-w+1:",size(iWnlist)[1]-w+1)
        g_iWn   = g_iWnlist[w]
        G_w     = GreenF_at_agrid(g_iWn,Kind,spin,SelfE_at_agrid(w,spin),testmu)
        G_w_t   = transpose(conj(G_w))
        #G_rev_w = transpose(conj(GreenF_at_agrid(g_iWnlist[end-w+1],Kind,spin,SelfE_at_agrid(g_iWnlist_size-w+1,spin),testmu)))
        #dummy=dummy - (1/beta)*(G_k_iWn[k1,k2,k3,w,:,:,s]       - Iden/iWnlist[w] - gtilda2/(iWnlist[w]*iWnlist[w]) - gtilda3/(iWnlist[w]*iWnlist[w]*iWnlist[w]) ) * exp(-iWnlist[w]*(beta-delta)) 
        #dummy=dummy - (1/beta)*(G_k_iWn[k1,k2,k3,end-w+1,:,:,s] + Iden/iWnlist[w] - gtilda2/(iWnlist[w]*iWnlist[w]) + gtilda3/(iWnlist[w]*iWnlist[w]*iWnlist[w]) ) * exp(iWnlist[w]*(beta-delta)) 
        dummy=dummy - (1/g_beta)*(G_w   - Iden/g_iWn - gtilda2/(g_iWn*g_iWn) - gtilda3/(g_iWn*g_iWn*g_iWn) ) * exp(-g_iWn*(g_beta-g_delta)) 
        dummy=dummy - (1/g_beta)*(G_w_t + Iden/g_iWn - gtilda2/(g_iWn*g_iWn) + gtilda3/(g_iWn*g_iWn*g_iWn) ) * exp( g_iWn*(g_beta-g_delta)) 
    end
...
function Cal_Occupation_givenMu_WOdc(iWnlist,H_k,SelfE_w,testmu,beta,delta,Corr_atom_Ind,Corr_orbital_Ind,Corr_atom_equiv)
    nk1, nk2, nk3               = size(H_k)[1:3]
    dimOrb1, dimOrb2, dimSpin    = size(H_k)[4:6]
    Occup=zeros(dimOrb1,dimOrb2,dimSpin)+zeros(dimOrb1,dimOrb2,dimSpin)*im
    H_loc=zeros(dimOrb1,dimOrb2,dimSpin)+zeros(dimOrb1,dimOrb2,dimSpin)*im
    # Iden=Matrix{ComplexF64}(I,dimOrb1,dimOrb2)
    ...
    for s= 1 : dimSpin
        ...
        Occup[:,:,s]=sum(dummy_list_tmp)/nktot
...
function ReadChemInteracting( arg_input_toml_dmft_jx )
    println("Reading TOML chem_int ...")
    try
        # @show arg_input_toml_dmft_jx ## arg_input_toml["DMFT_Jx"]
        chem_int    = arg_input_toml_dmft_jx.chem_int
        println(chem_int)
        println("")
        return chem_int
    catch
