...
using KaiEDJ: input_handler, show_input, write_outfile, write_outfile_mat
using DFTforge
using Distributed

@everywhere using KaiEDJ
@everywhere using KaiEDJ: Printf, TOML 
@everywhere using KaiEDJ: input_handler, show_input, write_outfile, write_outfile_mat
...
if( haskey( arg_input.Optional, "KaiEDJ" ) )
    header_dmft_jx = "KaiEDJ"
end
@show header_dmft_jx
arg_input_header_dmft_jx = arg_input.Optional[header_dmft_jx]

if  ( arg_input_header_dmft_jx.Calculation_mode == "DMFT" )
    ...
    , EDMFTF_MonteCar_step, EDMFTF_warmup_step, EDMFTF_GlobalFlip, EDMFTF_tsample, EDMFTF_nom, EDMFTF_PChangeOrder,SelfE_file) = input_handler(arg_input_header_dmft_jx);
    

    
elseif ((arg_input_header_dmft_jx.Calculation_mode == "Jx-DMFT") || (arg_input_header_dmft_jx.Calculation_mode == "Jx-DMFT-private") || (arg_input_header_dmft_jx.Calculation_mode == "DMFT+MFT"))
...
elseif (arg_input_header_dmft_jx.Calculation_mode == "Jx0" || arg_input_header_dmft_jx.Calculation_mode == "MFT")

    (Calculation_mode, BoltzmannC, delta, K_grid_num, R_grid_num, iW_grid_cut,Tem, beta, DMFT_spin_type,
                 Corr_atom_Ind, Corr_orbital_Ind, Corr_atom_equiv, Ineq_atom_Ind, Corr_ineq_orbital_Num,imp_ind,mag_order) = input_handler(arg_input_header_dmft_jx);
...
elseif (arg_input_header_dmft_jx.Calculation_mode == "Spinwave" || arg_input_header_dmft_jx.Calculation_mode == "Magnon")
...
if  ( Calculation_mode == "DMFT" )
    ...
    #----- run CTQMC -----#
    run_DMFT(DMFT_solver, DMFT_spin_type, Ineq_atom_Ind, imp_J, imp_ind, imp_int_type, arg_input_header_dmft_jx.mpi_prefix, regulated_Eimp)
...
if ( Calculation_mode == "Jx-DMFT"  || (Calculation_mode == "DMFT+MFT" && DMFT_solver!="ED" ) )

    println("#####################################################")
    println(" Calculation type : Jx calculation with self-energy (compatible with QMC)")
    println("#####################################################\n\n\n")
...
if ( Calculation_mode == "Jx-DMFT-private" ||  (Calculation_mode == "DMFT+MFT" && DMFT_solver=="ED" ) )

    println("#####################################################")
    println(" Calculation type : Jx calculation with self-energy (compatible with ED)")
    println("#####################################################\n\n\n")
    ...
    if (  !isfile("Nele.dat") || !isfile("transform_Mat.dat") || !isfile("DFT_CorrNele.dat"))
        ...
        println("Reading Nele.dat DFT_CorrNele.dat ...")
        targetN=parse(Float64, read_Nele())[1]
        dft_corrN= read_DFT_corrNele()     
        Trans_mat = read_transforMat()
        Trans_mat_A =Tvec_print_from_reading(H0_k, Trans_mat, Corr_atom_Ind, Corr_atom_equiv, Corr_orbital_Ind)
    ...
    #----- Find mu -----#
    arg_input_toml = TOML.parsefile(arg_input.TOMLinput)
    chem_int    = ReadChemInteracting( arg_input_header_dmft_jx )
    mu          += chem_int 
    ...
    Occup, Hloc = Cal_Occupation_givenMu_WOdc(iWnlist,H0_k,SelfE_w_sm,mu,beta,delta,Corr_atom_Ind,Corr_orbital_Ind,Corr_atom_equiv);
    check_write_occupancy( Occup, Corr_orbital_Ind, Corr_atom_Ind )
...
if ( Calculation_mode == "Jx0" || Calculation_mode == "MFT" )
                    ...
                    if matind == matind2
                        ...
                        if ( aind1[1] == aind2[1] )
                            for iatom = 1:n_Corr_atom_Ind
                                ...
                                dist[Rind[1]+1,Rind[2]+1,Rind[3]+1,iatom,iatom] = dist_dum
                            end
                        end
            ...
            for R2 = 1:R_grid_num[2]
                for R3 = 1:R_grid_num[3]
                    for (o1,o1val) in enumerate(Corr_atom_Ind)
                        for (o2,o2val) in collect(enumerate(Corr_atom_Ind))[o1:end]
                            if Neighbors_cut == 300
                                Neighbors_cut = size(ordered_dist[o1])[1]-1
                            end
                            if ( true ) # ( 0.0 < dist[R1,R2,R3,o1,o2] <= 20. )
                                # if (o1==o2) && (o2==2) 
                                #     println("inner : ", Jmatval )
                                # end
                                ...
                                # cellvec_fac_comp = cellvec_fac[R1,R2,R3,:]
                                # rel_pos_frac_comp = rel_pos_frac[o1,o2,:]
                                # cvec = (cellvec_fac_comp[1]+rel_pos_frac_comp[1])*cell_vec[1,:] 
                                #         + (cellvec_fac_comp[2]+rel_pos_frac_comp[2])*cell_vec[2,:] 
                                #         + (cellvec_fac_comp[3]+rel_pos_frac_comp[3])*cell_vec[3,:]

                                # R1inv, R2inv, R3inv = cellvec_minus_ind[R1,R2,R3,:]
                                # cellvec_fac_inversion_comp = cellvec_fac[R1inv,R2inv,R3inv,:]
                                # rel_pos_frac_inversion_comp = rel_pos_frac[o2,o1,:]              # Assumption. It should be changed for different systems where they are not a inverion partner.
                                # cvec_inv = (cellvec_fac_inversion_comp[1]+rel_pos_frac_inversion_comp[1])*cell_vec[1,:] 
                                #             + (cellvec_fac_inversion_comp[2]+rel_pos_frac_inversion_comp[2])*cell_vec[2,:] 
                                #             + (cellvec_fac_inversion_comp[3]+rel_pos_frac_inversion_comp[3])*cell_vec[3,:]
                                # if [R1,R2,R3]==[2,2,6]
                                #     @show (o1,o2)
                                #     @show [R1,R2,R3]
                                #     @show cellvec_fac[R1,R2,R3,:]
                                #     @show cellvec_minus_ind[R1,R2,R3,:]
                                ...
                                phase_phi12 = qpt_frac[qind,1]*(cellvec_fac[R1,R2,R3,1]+rel_pos_frac[o1,o2,1]) +
                                            qpt_frac[qind,2]*(cellvec_fac[R1,R2,R3,2]+rel_pos_frac[o1,o2,2]) +
                                            qpt_frac[qind,3]*(cellvec_fac[R1,R2,R3,3]+rel_pos_frac[o1,o2,3])
                                phase_phi12 *= 2.0*pi

                                phase_phi21 = -phase_phi12

                                phase_phi11 = qpt_frac[qind,1]*(cellvec_fac[R1,R2,R3,1]) +
                                            qpt_frac[qind,2]*(cellvec_fac[R1,R2,R3,2]) +
                                            qpt_frac[qind,3]*(cellvec_fac[R1,R2,R3,3])
                                phase_phi11 *= 2.0*pi

                                phase_phi22 = phase_phi11

                                # Factor-2 come from the formalism with H = \sum_ij J_ij ei ej 
                                J11 = 2.0 * Jmat[R1,R2,R3,o1,o1]
                                J12 = 2.0 * Jmat[R1,R2,R3,o1,o2]
                                J21 = 2.0 * Jmat[R1,R2,R3,o1,o2] # Symmetrize it with Parity-symmetry
                                J22 = 2.0 * Jmat[R1,R2,R3,o2,o2]

                                if [R1,R2,R3] == [1,1,1]    # onsite diagonal (=onsite-energy) should be ignored in formalism.
                                    J11 = 0.0
                                    J22 = 0.0
                                end
                                ...
                                Sz1  = 0.5 * Moment[o1]
                                Sz2  = 0.5 * Moment[o2]
                                # Sz2  = Sz1 * sign(Moment[o1]*Moment[o2])  # Symmetrized for real-eigenvalues


                                if (o1==o2)
                                    Magmat_[qind, o1,o1] += J11 / Sz1  # k=0 part

                                    original_fraction   = 1.0
                                    inversion_fraction  = 0.0
                                    Magmat_[qind, o1,o1] -= original_fraction * J11 / Sz1 * exp( -phase_phi11*im ) # different-site but same orbital basis
                                    Magmat_[qind, o1,o1] -= inversion_fraction * J11 / Sz1 * exp(  phase_phi11*im ) # different-site but same orbital basis  # inversion_symmetrizing partner
                                else
                                    Magmat_[qind, o1,o1] += J12 / Sz1  # k=0 part
                                    Magmat_[qind, o2,o2] += J21 / Sz2  # k=0 part

                                    original_fraction   = 1.0
                                    inversion_fraction  = 0.0
                                    Magmat_[qind, o1,o2] -= original_fraction  * J12 / Sz2 * exp( -phase_phi12*im )
                                    Magmat_[qind, o1,o2] -= inversion_fraction * J12 / Sz2 * exp(  phase_phi12*im )

                                    Magmat_[qind, o2,o1] -= original_fraction  * J21 / Sz1 * exp( -phase_phi21*im )
                                    Magmat_[qind, o2,o1] -= inversion_fraction * J21 / Sz1 * exp(  phase_phi21*im )
                                end
    
                                # if (o1==o2)
                                # else  # different-site and different-orbital 
                                #     if qmode == 1
                                #         # Magmat_[qind, o1,o2] -= 1.0 * Jmat[R1,R2,R3,o1,o2] / (0.5*Moment[o2]) * exp( dot(qvec, -cvec)*im )
                                #         # Magmat_[qind, o1,o2] -= 1.0 * Jmat[R1,R2,R3,o1,o2] / (0.5*Moment[o2]) * exp( dot(qvec,  cvec)*im )
                                #         # Magmat_[qind, o2,o1] -= 1.0 * Jmat[R1,R2,R3,o1,o2] / (0.5*Moment[o1]) * exp( dot(qvec,  cvec)*im )
                                #         # Magmat_[qind, o2,o1] -= 1.0 * Jmat[R1,R2,R3,o1,o2] / (0.5*Moment[o1]) * exp( dot(qvec, -cvec)*im )
...
if ( Calculation_mode == "Spinwave" || Calculation_mode == "Magnon" )
    println("    #############################################")
    ...
    if( isfile("occ.log") )
        println("Reading occu.log ...")
        Moment_size = loadmomentsize("occ.log", Corr_atom_equiv)  #magnetic moment size, unit :Bohr magneton
    else
        println("Reading occu_mag_tblatt.dat ...")
        Moment_size = readdlm("occu_mag_tblatt.dat")[2,:]
    end
