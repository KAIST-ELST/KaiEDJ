...
function SelfE_at_agrid(Wind,spin)
    ...
    for j = 1:size(g_Corr_atom_Ind)[1]
        ...
        g_Corr_atom_equiv_j     = g_Corr_atom_equiv[j]
        g_Corr_atom_equiv_j_abs = abs(g_Corr_atom_equiv_j)
        size_block  = size(g_imp_ind[g_Corr_atom_equiv_j_abs])[1]
        block_S     = zeros(size_block,size_block) + zeros(size_block, size_block)*im
        for l =1:g_Corr_ineq_orbital_Num[g_Corr_atom_equiv_j_abs]
            if g_Corr_atom_equiv_j >0
                block_S[findall(x->x==l,  g_imp_ind[g_Corr_atom_equiv_j_abs] )].= g_SelfE_w[g_Corr_atom_equiv_j_abs][Wind,l,spin]
            else
                block_S[findall(x->x==l,  g_imp_ind[g_Corr_atom_equiv_j_abs] )].= g_SelfE_w[g_Corr_atom_equiv_j_abs][Wind,l,mod(spin,2)+1]
            end
        end
