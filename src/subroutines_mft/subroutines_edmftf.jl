...
function EDMFTF_write_trans(Trans_mat_C,Corr_atom_equiv,DMFT_spin_type)
    ...
    for i in unique(abs.(Corr_atom_equiv))
        ...
        open(string("imp_",i,"/Trans.dat"), "w") do io    
            Printf.@printf(io,"%3i %3i #  size of Sigind and CF",size(Trans_mat_C[1])[1]*DMFT_spin_type, size(Trans_mat_C[1])[1]*DMFT_spin_type)
            Printf.@printf(io,"\n")
            Printf.@printf(io,"#---- Sigind follows")
            Printf.@printf(io,"\n")
            ...
            for r = 1:size(Trans_mat_C[ai])[1]*DMFT_spin_type
                for c =1:size(Trans_mat_C[ai])[1]*DMFT_spin_type
                    ...
                    if (r <(size(imp_ind[i])[1] +1) && c <(size(imp_ind[i])[2] +1))
                        Printf.@printf(io,"%3i", imp_ind[i][r,c])
                    elseif (r <(size(imp_ind[i])[1] +1) && c >(size(imp_ind[i])[2] ))
                        Printf.@printf(io,"%3i", 0)
                    ...
                    elseif (r >(size(imp_ind[i])[1]) && c >(size(imp_ind[i])[2] ))
                        if (imp_ind[i][r-size(imp_ind[i])[1], c-size(imp_ind[i])[2]] == 0)         
                            ...
                            Printf.@printf(io,"%3i", imp_ind[i][r-size(imp_ind[i])[1], c-size(imp_ind[i])[2]]+maximum(imp_ind[i]))
                        end
                    end
                end
                Printf.@printf(io,"\n")
            ...
            for r = 1:size(Trans_mat_C[ai])[1]*DMFT_spin_type
                for c =1:size(Trans_mat_C[ai])[1]*DMFT_spin_type
                    ...
                    if (r <(size(imp_ind[i])[1] +1) && c <(size(imp_ind[i])[2] +1))
                        Printf.@printf(io,"%13.9f %13.9f ", real(Trans_mat_C[ai][r,c]), imag(Trans_mat_C[ai][r,c]) )
                    elseif (r <(size(imp_ind[i])[1] +1) && c >(size(imp_ind[i])[2] ))
                        Printf.@printf(io,"%13.9f %13.9f ", 0.0, 0.0 )
                    elseif (r >(size(imp_ind[i])[1]) && c <(size(imp_ind[i])[2] )+1)
                        Printf.@printf(io,"%13.9f %13.9f ", 0.0, 0.0 )
                    elseif (r >(size(imp_ind[i])[1]) && c >(size(imp_ind[i])[2] ))
                        Printf.@printf(io,"%13.9f %13.9f ", real(Trans_mat_C[ai][r-size(imp_ind[i])[1],c-size(imp_ind[i])[2]]), imag(Trans_mat_C[ai][r-size(imp_ind[i])[1],c-size(imp_ind[i])[2]] ) )
                    end
                end
                Printf.@printf(io,"\n")
            end            
...
function EDMFTF_write_PARAMS(Corr_atom_equiv,imp_U,imp_J,imp_dc_n,beta,imp_int_type,EDMFTF_MonteCar_step,EDMFTF_GlobalFlip,EDMFTF_warmup_step,mu,regulated_Eimp,regulated_ShiftE,EDMFTF_tsample,EDMFTF_nom,EDMFTF_PChangeOrder)
    ...
    for i in unique(abs.(Corr_atom_equiv))
        open(string("imp_",i,"/PARAMS"), "w") do io
            Printf.@printf(io,"OffDiagonal  real\n")
            Printf.@printf(io,"aom %2i\n", 1)
            Printf.@printf(io,"Sig  Sig.out\n")
            Printf.@printf(io,"Delta  Delta.inp\n")
            Printf.@printf(io,"cix  actqmc.cix\n")

            Printf.@printf(io,"Ncout %8i\n",1000000)
            Printf.@printf(io,"nom %5i\n",EDMFTF_nom)
            Printf.@printf(io,"svd_lmax %3i\n",25)
            Printf.@printf(io,"exe  ctqmc\n")
            Printf.@printf(io,"tsample %5i\n",EDMFTF_tsample)
            Printf.@printf(io,"warmup %14.2f\n", EDMFTF_warmup_step)
            Printf.@printf(io,"Ed  [")
                ...
                for s = 1:DMFT_spin_type
                    for ii in unique(diag(imp_ind[i]))
                        if ((s != DMFT_spin_type) || (ii != maximum(diag(imp_ind[i])) ))
                            Printf.@printf(io,"%14.9f,", regulated_Eimp[i][findall(x->x==ii, diag(imp_ind[i]))[1]]+regulated_ShiftE[i]-mu )
                        else
                            Printf.@printf(io,"%14.9f]\n", regulated_Eimp[i][findall(x->x==ii, diag(imp_ind[i]))[1]]+regulated_ShiftE[i]-mu )
                        end
                    end
                end
            end
            ...
            if EDMFTF_PChangeOrder != 0.9
                Printf.@printf(io,"PChangeOrder  %f\n", EDMFTF_PChangeOrder)
            end
            Printf.@printf(io,"U  %f\n", imp_U[i])                
            Printf.@printf(io,"J  %f\n", imp_J[i])
            Printf.@printf(io,"M  %14.2f\n", EDMFTF_MonteCar_step)

            if (imp_int_type[i] == "full" || imp_int_type[i] == "Full" || imp_int_type[i] == "FULL")       
                Printf.@printf(io,"CoulombF  '%s'\n", "Full")
            elseif(imp_int_type[i] == "ising" || imp_int_type[i] == "Ising" || imp_int_type[i] == "ISING") 
                Printf.@printf(io,"CoulombF  '%s'\n", "Ising")            
            end
                
            Printf.@printf(io,"mu  %14.9f\n", -regulated_Eimp[i][findall(x->x==1, diag(imp_ind[i]))[1]]-regulated_ShiftE[i]+mu )
            Printf.@printf(io,"beta  %14.9f\n", beta)                
            Printf.@printf(io,"mode  SM\n")                
            Printf.@printf(io,"GlobalFlip  %i\n", EDMFTF_GlobalFlip)
            Printf.@printf(io, "nf0  %f\n", imp_dc_n[i])
